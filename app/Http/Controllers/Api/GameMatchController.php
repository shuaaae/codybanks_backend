...
class GameMatchController extends Controller
{
    ...
    /**
     * Get matches with player assignments for hero statistics calculation
     */
    public function getMatchesWithAssignments(Request $request)
    {
        try {
            // Get team_id from query parameter first
            $teamId = $request->query('team_id');
            ...
            // Debug logging
            \Log::info('GET /api/matches-with-assignments called', [
                'query_team_id' => $request->query('team_id'),
                'parsed_team_id' => $teamId,
                'match_type' => $matchType
            ]);
            ...
            // Get matches with player assignments
            $matches = \App\Models\GameMatch::where('team_id', $teamId)
                ->where('match_type', $matchType)
                ->with([
                    'teams:id,match_id,team,team_color',
                    'playerAssignments' => function($query) use ($teamId) {
            ...
            // Transform the data to include player assignments in a format suitable for hero stats calculation
            $transformedMatches = $matches->map(function($match) {
                ...
                foreach ($match->teams as $team) {
                    ...
                    // Get player assignments for this specific team
                    // Since we're filtering by team_id in the query, all assignments belong to our team
            ...
            \Log::info('Matches with assignments result', [
                'team_id' => $teamId,
                'match_type' => $matchType,
                'matches_count' => $transformedMatches->count()
            ]);

            return response()->json($transformedMatches);
        } catch (\Throwable $e) {
            \Log::error('GET /api/matches-with-assignments failed', [
                'message' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
            ]);
