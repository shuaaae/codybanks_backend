...
// New endpoint for combined match data with player assignments
Route::get('/matches-with-assignments/{teamId}', function ($teamId) {
    try {
        $matchType = request()->query('match_type', 'scrim');
        
        \Log::info('Fetching matches with assignments', [
            'team_id' => $teamId,
            'match_type' => $matchType
        ]);
        
        // Get matches with teams and player assignments
        $matches = \App\Models\GameMatch::where('team_id', $teamId)
            ->where('match_type', $matchType)
            ->with([
                'teams' => function($query) {
                    $query->select('id', 'match_id', 'team', 'team_color', 'picks1', 'picks2', 'banning_phase1', 'banning_phase2');
                },
                'teams.playerAssignments' => function($query) use ($teamId) {
                    $query->whereHas('player', function($q) use ($teamId) {
        ...
        // Transform the data to include player assignments in picks
        $transformedMatches = $matches->map(function($match) {
            $transformedTeams = $match->teams->map(function($team) {
                // Get player assignments for this team
                $assignments = $team->playerAssignments->groupBy('role');
                
                // Transform picks to include player data from assignments
                $transformedPicks1 = collect($team->picks1 ?? [])->map(function($pick, $index) use ($assignments) {
                    // Try to find player assignment for this pick based on index/role
                    $roles = ['exp', 'jungler', 'mid', 'gold', 'roam'];
                    $role = $roles[$index] ?? 'unknown';
                    
                    if (isset($assignments[$role])) {
                        $assignment = $assignments[$role]->first();
                        if ($assignment) {
                            return [
                ...
                $transformedPicks2 = collect($team->picks2 ?? [])->map(function($pick, $index) use ($assignments) {
                    // For picks2, we need to offset the role index
                    $roles = ['exp', 'jungler', 'mid', 'gold', 'roam'];
                    $role = $roles[$index] ?? 'unknown';
                ...
                return [
                    ...
                    'picks1' => $transformedPicks1,
                    'picks2' => $transformedPicks2,
        ...
        \Log::info('Returning transformed matches', [
            'count' => $transformedMatches->count(),
            'sample' => $transformedMatches->first()
        ]);
        ...
        \Log::error('Error fetching matches with assignments', [
            'team_id' => $teamId,
            'error' => $e->getMessage(),
            'trace' => $e->getTraceAsString()
        ]);
        
        return response()->json(['error' => 'Failed to fetch matches with assignments'], 500)
            ->header('Access-Control-Allow-Origin', '*')
            ->header('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS')
            ->header('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With');
    }
